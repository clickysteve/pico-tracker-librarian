...
- **Audio-reactive effects** — drive the effects from a live audio input. Pick the interface the picoTracker (or your mixer) is plugged into and the picture moves with what you are hearing; there is no sync problem because it is the real signal, a frame or two behind. Bass, mids, highs, level and a transient pulse are each available as a source, and every effect with a meaningful "more" can be wired to one with a depth from −100% to +100%, so it can duck on a beat as well as swell on one. Your sliders stay where you put them and the modulation rides on top. Nothing is routed to the speakers, and no audio device is opened without a click.
...
```bash
node tests/parser.test.mjs   # 160 unit tests: formats, MIDI timing, slice/loop units, round-trips, generators, scales, audio analysis
...
node tests/e2e.mjs           # 239 browser checks (needs: npm i -D playwright)
```
